...
def compute_sentiment_map(pdf_iter):
    ...
    for pdf in pdf_iter:
        if not len(pdf):
            ...
            continue
        pdf = pdf.copy()
        texts = pdf.get("review_text_clean", pd.Series([], dtype=str)).astype(str)
        pdf["sentiment_compound"] = texts.map(lambda t: sia.polarity_scores(t)["compound"]).astype(float)
        yield pdf


def main() -> None:
    parser = argparse.ArgumentParser(description="Review sentiment analysis CLI (PySpark)")
    ...
    # helpful ratio
    total_safe = F.when(F.col("total_votes").isNull() | (F.col("total_votes") == 0), F.lit(1)).otherwise(F.col("total_votes"))
    df = df.withColumn("helpful_ratio", (F.col("helpful_yes").cast("float") / total_safe).cast("float"))

    # compute sentiment with mapInPandas
    schema = T.StructType([*(df.schema.fields), T.StructField("sentiment_compound", T.FloatType(), True)])
    ...
    summary.coalesce(1).write.mode("overwrite").option("header", "true").csv(args.output_csv)
